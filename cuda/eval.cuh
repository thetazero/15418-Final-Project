#ifndef EVAL
#define EVAL
__device__ __inline__ int idx(int r, int c, int size) { return r * size + c; }
...
__device__ __inline__
void scan_right(int size, char *board, char *x_scratch, char *o_scratch) {
  int state = 0;
...
__device__ __inline__
void scan_left(int size, char *board, char *x_scratch, char *o_scratch) {
  int state = 0;
  int r0 = 0, c0 = 0;
  for (int c=c0; c < size; c++){
    for (int r=r0; r < size; r++) {
